// wasm dlmalloc: sbrk + Doug Lea malloc 2.8.6 (wasi-libc wasm patches).
...
#define CWASM_PAGE_SIZE 65536u

__attribute__((import_module("env"), import_name("__grow_memory")))
int __grow_memory(unsigned pages);

void *sbrk(intptr_t increment) {
    if (increment == 0)
        return (void *)(__builtin_wasm_memory_size(0) * CWASM_PAGE_SIZE);

    if (increment % (intptr_t)CWASM_PAGE_SIZE != 0 || increment < 0)
        abort();

    size_t old = __builtin_wasm_memory_size(0);
    if (__grow_memory((unsigned)(increment / (intptr_t)CWASM_PAGE_SIZE)) != 0) {
        errno = ENOMEM;
        return (void *)-1;
    }
    return (void *)(old * CWASM_PAGE_SIZE);
}
...
#if !defined(USE_LOCKS) && defined(CWASM_THREADS)
#define USE_LOCKS 1
...
static void try_init_allocator(void) {
  ...
  char *end = &__heap_end;
  if (end < base)
    end = (char *)(uintptr_t)(__builtin_wasm_memory_size(0) * 65536u);
...
void *aligned_alloc(size_t alignment, size_t size) {
    if (alignment == 0 || size == 0 || (alignment & (alignment - 1u)) != 0 ||
        alignment % sizeof(void *) != 0 || (size % alignment) != 0) {
        errno = EINVAL;
        return NULL;
    }
